...
// Global state
let allChecks = [];
let summaryDataFull = null; // Full summary (all data)
let summaryDataMaintExcluded = null; // Maintenance-excluded summary (2am-5am filtered out)
...
async function refreshCsrfToken() {
    const sessionResp = await fetch("/api/auth/session_check", {
...
function showPasswordChangeDialog() {
    ...
    function checkPasswordRequirements(password) {
        ...
        if (lengthEl) {
            lengthEl.className = requirements.length
                ? "pwd-req-met"
                : "pwd-req-unmet";
            lengthEl.textContent =
                (requirements.length ? "✔️ " : "❌ ") +
                "At least 12 characters";
        }
        if (uppercaseEl) {
            uppercaseEl.className = requirements.uppercase
                ? "pwd-req-met"
                : "pwd-req-unmet";
            uppercaseEl.textContent =
                (requirements.uppercase ? "✔️ " : "❌ ") +
                "One uppercase letter";
        }
        if (lowercaseEl) {
            lowercaseEl.className = requirements.lowercase
                ? "pwd-req-met"
                : "pwd-req-unmet";
            lowercaseEl.textContent =
                (requirements.lowercase ? "✔️ " : "❌ ") +
                "One lowercase letter";
        }
        if (digitEl) {
            digitEl.className = requirements.digit
                ? "pwd-req-met"
                : "pwd-req-unmet";
            digitEl.textContent =
                (requirements.digit ? "✔️ " : "❌ ") + "One digit";
        }
        if (specialEl) {
            specialEl.className = requirements.special
                ? "pwd-req-met"
                : "pwd-req-unmet";
            specialEl.textContent =
                (requirements.special ? "✔️ " : "❌ ") +
    ...
    function updatePasswordStrength(password) {
        ...
        const themeColors = {
            neutral:
                style.getPropertyValue("--border-color").trim() || "#e0e0e0",
...
// Sync Flatpickr theme with dashboard theme
...
function initializeDatePickers() {
    // Sync Flatpickr dark theme with dashboard theme on init
    syncFlatpickrTheme();

    const commonConfig = {
        enableTime: true,
        dateFormat: "Y-m-d\\TH:i", // API-compatible format with T separator
        time_24hr: false, // Show AM/PM (12-hour format)
        allowInput: true,
        altInput: true,
        altFormat: "M j, Y h:i K", // Human-readable format: "Dec 31, 2025 11:59 PM"
    };

    // Start date picker - defaults to start of day when date selected
    startDatePicker = flatpickr("#startDate", {
        ...commonConfig,
        defaultHour: 0,
        defaultMinute: 0,
        onChange: function (selectedDates, dateStr) {
            // If end date picker has a date, ensure start is before end
            if (endDatePicker && endDatePicker.selectedDates.length > 0) {
                const endDate = endDatePicker.selectedDates[0];
                if (selectedDates.length > 0 && selectedDates[0] > endDate) {
                    // Start is after end - adjust start to be before end
    ...
    // End date picker - defaults to end of day when date selected
    endDatePicker = flatpickr("#endDate", {
        ...commonConfig,
        defaultHour: 23,
        defaultMinute: 59,
        onChange: function (selectedDates, dateStr) {
            // If start date picker has a date, ensure end is after start
            if (startDatePicker && startDatePicker.selectedDates.length > 0) {
                const startDate = startDatePicker.selectedDates[0];
                if (selectedDates.length > 0 && selectedDates[0] < startDate) {
                    // End is before start - adjust end to be after start
...
// Set default date range to last 14 days with time (start: 00:00, end: 23:59)
function setDefaultDateRange() {
    ...
    document.getElementById("startDate").value = formatDateTime(
        startDate,
        false,
    );
    document.getElementById("endDate").value = formatDateTime(endDate, true);
}
...
async function loadModemList() {
    try {
        ...
        if (!response.ok) {
            ...
            showToast(getErrorMessage(response, data), "error");
            return;
        }
        ...
        if (!data.modems || data.modems.length === 0) {
            ...
            showToast("No modems found. Upload some data first.", "error");
            return;
        }
        ...
        data.modems.forEach((modem) => {
            ...
            const displayText = modem.modem_type
                ? `${modem.modem_type} - ${modem.modem_id.split("-").pop()}`
                : modem.modem_id;
            option.textContent = displayText;
            option.addEventListener("click", () =>
                selectModem(modem.modem_id, displayText),
            );
        ...
        showToast("Unable to load modem list. Please try again.", "error");
    }
}
...
async function loadData() {
    ...
    if (!modemId) {
        showToast("Please select a modem", "error");
        return;
    }
    ...
    // Disable view buttons until all data is loaded
    const viewBtns = document.querySelectorAll(".view-btn");
    viewBtns.forEach((btn) => (btn.disabled = true));
    ...
    try {
        ...
        // Start all three requests in parallel for faster loading
        ...
        // Process summary data (dual summaries: full + maintenance-excluded)
        const summaryResult = await summaryResponse.json();
        if (summaryResult.success) {
            summaryDataFull = summaryResult.full;
            summaryDataMaintExcluded = summaryResult.maint_excluded;
        }
        ...
        // Show Summary View by default (if we have summary data)
        // Use full data by default (maintenance toggle is OFF by default)
        const defaultSummary = summaryDataFull || summaryDataMaintExcluded;
        if (defaultSummary) {
            showSummaryView();
            displaySummaryData();
            // Show content area now that data is loaded
            document.getElementById("contentArea").classList.remove("no-data");
        ...
        viewBtns.forEach((btn) => (btn.disabled = false));
    }
}
...
function showSummaryView() {
    ...
    const viewBtns = document.querySelectorAll(".view-btn");
    viewBtns[0].classList.add("active"); // Summary
    viewBtns[1].classList.remove("active"); // Detail View
    viewBtns[2].classList.remove("active"); // Trend View

    document.getElementById("timelineNav").classList.remove("active");

    // Display summary data if available
    if (summaryDataFull || summaryDataMaintExcluded) {
        displaySummaryData();
    }
}
...
function showDetailView() {
    ...
    const viewBtns = document.querySelectorAll(".view-btn");
    viewBtns[0].classList.remove("active"); // Summary
    viewBtns[1].classList.add("active"); // Detail View
    viewBtns[2].classList.remove("active"); // Trend View
...
function showTrendsView() {
    ...
    viewBtns[0].classList.remove("active"); // Summary
    viewBtns[1].classList.remove("active"); // Detail View
    viewBtns[2].classList.add("active"); // Trend View
...
// Toggle maintenance window exclusion
function toggleMaintenance() {
    const btn = document.getElementById("excludeMaintenanceToggle");
    btn.classList.toggle("active");
    displaySummaryData();
}

// Toggle analysis (placeholder for future functionality)
function toggleAnalysis() {
    const btn = document.getElementById("enableAnalysisToggle");
    btn.classList.toggle("active");
    // Future: Add analysis toggle logic here
}

// Display summary data in the Summary View tables
function displaySummaryData() {
    // Select data source based on maintenance toggle (button with .active class = exclude maintenance)
    const excludeMaint =
        document
            .getElementById("excludeMaintenanceToggle")
            ?.classList.contains("active") ?? false;
    const summaryData = excludeMaint
        ? summaryDataMaintExcluded
        : summaryDataFull;
    ...
    if (period) {
        document.getElementById("sum-total-checks").textContent = fmtInt(
            period.total_checks,
        );
        document.getElementById("sum-period-start").textContent =
            period.period_start ? formatDateTime(period.period_start) : "-";
        document.getElementById("sum-period-end").textContent =
            period.period_end ? formatDateTime(period.period_end) : "-";
        document.getElementById("sum-speedtest-count").textContent = fmtInt(
            period.checks_with_speedtest,
        );
        document.getElementById("sum-reboots").textContent = fmtInt(
            period.detected_reboots,
        );
    }

    // Network Performance Summary (first after Period Overview)
    const net = summaryData.network;
    if (net) {
        populateRow("sum-dl", net.download_speed);
        populateRow("sum-ul", net.upload_speed);
        populateRow("sum-ping-google", net.ping_google);
        populateRow("sum-ping-cf", net.ping_cloudflare);
        populateRow("sum-loss-google", net.loss_google);
        populateRow("sum-loss-cf", net.loss_cloudflare);
        populateRow("sum-jitter-google", net.jitter_google);
        populateRow("sum-jitter-cf", net.jitter_cloudflare);
    }

    // RX Signal Summary (includes BER metrics)
    const rx = summaryData.rx_signal;
    if (rx) {
        populateRow("sum-scqam-power", rx.scqam_power);
        populateRow("sum-scqam-snr", rx.scqam_snr);
        populateRow("sum-ofdm-power", rx.ofdm_power);
        populateRow("sum-ofdm-snr", rx.ofdm_snr);
    }
    ...
    // TX Signal Summary
    const tx = summaryData.tx_signal;
    if (tx) {
        populateRow("sum-tx-scqam", tx.scqam_power);
        populateRow("sum-tx-ofdma", tx.ofdma_power);
    }
}
...
function updateTimelineNav() {
    ...
    document.getElementById("nextBtn").disabled =
        currentCheckIndex === allChecks.length - 1;
...
function navigatePrevious() {
    if (currentCheckIndex > 0) navigateToIndex(currentCheckIndex - 1);
}
function navigateNext() {
    if (currentCheckIndex < allChecks.length - 1)
        navigateToIndex(currentCheckIndex + 1);
}
function navigateToLast() {
    navigateToIndex(allChecks.length - 1);
}
...
function displayCurrentCheck() {
    ...
    document.getElementById("modemtype").textContent =
        data.sysinfo?.modemtype || "-";
    document.getElementById("modemmac").textContent =
        data.sysinfo?.modemmac || "-";
    document.getElementById("firmware").textContent =
        data.sysinfo?.firmware || "-";
    document.getElementById("uptime").textContent = formatUptime(
        data.sysinfo?.uptime,
    );
    document.getElementById("systime").textContent = formatEpochTime(
    ...
    if (data.speedtest_latency) {
        document.getElementById("speedtest_ping").textContent =
            `${data.speedtest_latency.toFixed(1)} ms`;
    } else {
        document.getElementById("speedtest_ping").textContent = "-";
    }
    ...
    if (data.speedtest_max_latency) {
        speedTestPingParts.push(
            `Max: ${data.speedtest_max_latency.toFixed(1)} ms`,
        );
    }
    if (data.speedtest_jitter) {
        speedTestPingParts.push(
            `Jitter: ${data.speedtest_jitter.toFixed(1)} ms`,
        );
    }
    document.getElementById("speedtest_ping_details").textContent =
        speedTestPingParts.join(" | ");

    // Display download speed and loaded latency
    document.getElementById("iperf3_dl").textContent = formatSpeed(
        data.iperf3test_dl,
    );
    const dlLoadedParts = [];
    if (data.speedtest_dl_latency) {
        dlLoadedParts.push(
            `Loaded latency: ${data.speedtest_dl_latency.toFixed(1)} ms`,
        );
    }
    document.getElementById("iperf3_dl_loaded").textContent =
        dlLoadedParts.join(" | ");

    // Display upload speed and loaded jitter
    document.getElementById("iperf3_ul").textContent = formatSpeed(
        data.iperf3test_ul,
    );
    const ulLoadedParts = [];
    if (data.speedtest_ul_jitter) {
        ulLoadedParts.push(
            `Loaded jitter: ${data.speedtest_ul_jitter.toFixed(1)} ms`,
        );
    }
    document.getElementById("iperf3_ul_loaded").textContent =
        ulLoadedParts.join(" | ");
    ...
    if (pingGoogleAvg !== "-" && pingGoogleAvg !== "Failed") {
        document.getElementById("ping_google").textContent =
            `${parseFloat(pingGoogleAvg).toFixed(1)} ms`;
    } else {
    ...
    if (pingGoogleJitter && pingGoogleJitter !== "N/A") {
        googleParts.push(
            `Jitter: ${parseFloat(pingGoogleJitter).toFixed(1)} ms`,
        );
    }
    ...
    if (googleTopLineParts) {
        pingGoogleDetailsEl.appendChild(
            document.createTextNode(googleTopLineParts),
        );
    }
    if (pingGoogleLoss && pingGoogleLoss !== "N/A") {
        ...
        pingGoogleDetailsEl.appendChild(
            document.createTextNode("Loss: " + pingGoogleLoss),
        );
        ...
        if (data.traceroute_google && data.traceroute_google.status) {
            ...
            pingGoogleDetailsEl.appendChild(
                createTracerouteLink(data.traceroute_google),
            );
        }
    ...
    if (pingCloudflareAvg !== "-" && pingCloudflareAvg !== "Failed") {
        document.getElementById("ping_cloudflare").textContent =
            `${parseFloat(pingCloudflareAvg).toFixed(1)} ms`;
    } else {
        document.getElementById("ping_cloudflare").textContent =
            pingCloudflareAvg;
    }
    ...
    if (pingCloudflareMax && pingCloudflareMax !== "N/A") {
        cloudflareParts.push(
            `Max: ${parseFloat(pingCloudflareMax).toFixed(1)} ms`,
        );
    }
    if (pingCloudflareJitter && pingCloudflareJitter !== "N/A") {
        cloudflareParts.push(
            `Jitter: ${parseFloat(pingCloudflareJitter).toFixed(1)} ms`,
        );
    }
    ...
    document.getElementById("ping_cloudflare_details").textContent =
        cloudflareDetailsLines.join("\n");

    populateTable("rxTable", data.rx, [
        "portid",
        "frequency",
        "power",
        "snr",
    ...
    populateTable("rxofdmTable", data.rxofdm, [
        "portid",
        "subcarr0freq",
        "plclock",
        "ncplock",
    ...
    populateTable("txofdmTable", data.txofdm, [
        "portid",
        "state",
        "subcarr0freq",
        "power",
...
function parseSpeed(speed) {
    ...
    if (!speed || speed === "-" || speed === "Failed" || speed === "Disabled")
        return null;
...
function renderTrendChartsFromChecks() {
    if (allChecks.length < 1) {
        showToast("No data available to display trends", "error");
        return;
    }
    ...
    const speedtestMaxLatency = allChecks.map(
        (c) => c.speedtest_max_latency || null,
    );
    const googleMaxLatency = allChecks.map((c) => {
        const val = c.ping_google_max_latency;
        return val && val !== "N/A" && val !== "Failed"
            ? parseFloat(val)
            : null;
    });
    ...
    const rxScqamPowerData = allChecks.map((c) => {
        if (!c.rx || c.rx.length === 0)
            return { min: null, avg: null, max: null };
        const powers = c.rx
            .map((ch) => parseFloat(ch.power))
    ...
    const rxOfdmPowerData = allChecks.map((c) => {
        if (!c.rxofdm || c.rxofdm.length === 0)
            return { min: null, avg: null, max: null };
        const powers = c.rxofdm
            .map((ch) => parseFloat(ch.plcpower))
            .filter((p) => !isNaN(p) && isFinite(p));
        if (powers.length === 0) return { min: null, avg: null, max: null };
        return {
            min: Math.min(...powers),
    ...
    const txScqamData = allChecks.map((c) => {
        if (!c.tx || c.tx.length === 0)
            return { min: null, avg: null, max: null, bonded: 0 };
        const powers = c.tx
            .map((ch) => parseFloat(ch.power))
    ...
    const txOfdmaData = allChecks.map((c) => {
        if (!c.txofdm || c.txofdm.length === 0)
            return { bonded: 0, impaired: 0, avgPower: null };

        const operateStates = ["OPERATE", "Locked"];
        const impairedStates = [
            "Not Locked",
            "RNG1",
            "RNG2",
            "RNG3",
    ...
    const datasets = [
        {
            ...
            data: timestamps
                .map((t, i) => ({ x: t * 1000, y: downloadData[i] }))
        ...
        },
        {
            label: "Upload Speed",
            data: timestamps
                .map((t, i) => ({ x: t * 1000, y: uploadData[i] }))
                .filter((point) => point.y !== null && point.y !== undefined),
            borderColor: "#764ba2",
